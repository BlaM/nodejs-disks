...
function detail(drive, callback) {
    async.series(
        ...
        function (err, results) {
            if (err) return callback(err);
            results.freePer = numeral(results.available / (results.used + results.available) * 100).value()
            results.usedPer = numeral(results.used / (results.used + results.available) * 100).value()
            results.total = numeral(results.used + results.available).value();
            results.used = numeral(results.used).value();
            results.available = numeral(results.available).value();
            results.drive = drive;
